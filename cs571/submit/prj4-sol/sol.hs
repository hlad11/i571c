...
deptEmployees list d =
  if (emplDept (head list)) == d
  then (head list):(deptEmployees (tail list) d)
  else deptEmployees (tail list) d
...
employeesSalarySum list = sum $ map (\e -> emplSalary e) list
...
expn1 :: (Floating a)  => a -> Int -> [a]
expn1 x n = answer
  where z = (x**nFl)/denom
        nFl = fromIntegral n
        denom = fromIntegral $ fac n
        zs = expn1 x (n+1)
        answer = z:zs
...
charIndexes str char = answer
  where pairs = zip str [0..]
        occurPairs = filter (\(c, _) -> char == c) pairs
        answer = map (\(c, index) -> index) occurPairs
...
foldTree treeFn leafFn tree =
  case tree of
    Leaf val -> leafFn val
    Tree leftT val rightT ->
      treeFn (foldTree treeFn leafFn leftT) val (foldTree treeFn leafFn rightT)
...
flattenTree tree = foldTree (\l val r -> l ++ [val] ++ r) (\val -> [val]) tree
